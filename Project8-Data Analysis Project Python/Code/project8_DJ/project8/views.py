...
def search(request):
    if request.method == 'POST':
        ...
        teachers = Teacher.objects.raw(f"SELECT * FROM project8_teacher WHERE full_name = '{full_name}' LIMIT 20")
        drop = Teacher.objects.values('full_name').distinct()
    else:
        teachers = Teacher.objects.all()[:10]
        drop = Teacher.objects.values('full_name').distinct()
    return render(request,"pages/search.html", {'teachers':teachers, "drop":drop})
